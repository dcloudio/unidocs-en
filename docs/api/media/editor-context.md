...
Developers can choose to upload images to the server during the submission phase, and replace them after obtaining the web address. When replacing, the src value of `img` should be replaced for html content, and the value of `insert { image: abc }` should be replaced for delta content.
...
Initiate the editor content. Only delta comes into force when html and delta exist at the same time.
