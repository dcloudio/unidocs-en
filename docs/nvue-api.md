...
```
 ...
 - Third-party native plug-ins that support native cloud packaging in the project nativeplugins directory and plug-in market. It is also very convenient for you to transplant the existing native modules to nvue platform.
  ...
  Usage: can only be used in manifest.json->App native plug-in configuration-> select local plug-in or cloud plug-in-> package the custom base. [See details](/api/extend/native-plugin.md)
 ...
 - The uni-app API that is not supported in nvue, [See details](/nvue-api?id=nvue-%E9%87%8C%E4%B8%8D%E6%94%AF%E6%8C%81%E7%9A%84-uni-app-api)
...
> If you need to use CSS animation, please refer to [transition](/nvue-css?id=transition) and [transform](/nvue-css?id=transform).
...
**Note when using this page to communicate: use uni.$off to remove the event listener before unloading the page.** [Reference](https://uniapp.dcloud.io/collocation/frame/communication?id=off)
...
3. **globalData:** The `globalData` mechanism is universal at all ends. Define `globalData` in the App.vue file as follows:
...
| uni.pageScrollTo()| Scroll the page to the target location| [scrollToElement](#scrollToElement)|
