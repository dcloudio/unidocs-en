...
| services| Array\<String>| | No| uuid list of Bluetooth device main service to be searched. Some Bluetooth devices can broadcast the uuid of their main service. If this parameter is set, only search for Bluetooth devices whose broadcast packet has the main service corresponding to uuid. It is recommended to filter out other Bluetooth devices whose periphery doesn't need to be processed mainly by this parameter.|
...
* Currently, the App side only supports ble Bluetooth devices. For more Bluetooth devices, you can get the [Native plug-in](https://ext.dcloud.net.cn/search?q=%E8%93%9D%E7%89%99&cat1=5&cat2=51&orderBy=UpdatedDate) in the plug-in market.
