...
export default {
	setup (){
		// 在生命周期中使用时，为组件级别作用域，不指定第二个参数key的情况下，编译器会自动补充(默认，以文件+函数位置做base64生成key)
...
2. For cloud function binding custom urlized domain name, please refer to the document [Cloud Function Url normalization](https://uniapp.dcloud.net.cn/uniCloud/http)
