...
Since HXuilderX1.4, the uni-app has supported the introduction of native plug-ins and the usage is as follows:
...
You can log in [uni native plug-in market](https://ext.dcloud.net.cn/?cat1=5&cat2=51), click "Download for Offline Packaging" on the free plug-in details page to download the native plug-in (in zip format), and unzip it to the "nativeplugins" directory (if it does not exist, create it) under the uni-app project of HBuilderX. The following is an example of "DCloud-RichAlert" plug-in whose download address is: https://ext.dcloud.net.cn/plugin?id=36
...
1. Introduce the native plug-in uni.requireNativePlugin into the page, and return an object.

```js
const dcRichAlert = uni.requireNativePlugin('DCloud-RichAlert')
```
2. 使用原生插件
2. Use native plug-ins
...
```js
	dcRichAlert.show({
		...
		titleColor: '#FF0000',
		content: "<a href='https://uniapp.dcloud.io/' value='Hello uni-app'>uni-app</a> is a front-end framework using Vue.js to develop the cross-platform applications!\nFor free!\nFor free!\nFor free!\nRepeat thrice to emphasize",
		contentAlign: 'left',
		checkBox: {
...
1. More plugins are available in the plug-in market. Please refer to [uni native plug-in development document](https://nativesupport.dcloud.net.cn/NativePlugin/README) for developing uni native plug-ins.
2. 如果插件需要传递文件路径，则需要传手机文件的绝对路径。
2. If the plug-in needs to transmit the file path, the absolute path on the mobile phone file is required.
