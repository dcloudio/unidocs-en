...
| data| Object/String/ArrayBuffer| No| | Requested parameters| App does not support ArrayBuffer type|
...
| responseType| String| No| text| Set the data type of response. Legal values: text, arraybuffer| |
|sslVerify|Boolean|否|true|验证 ssl 证书|仅App安卓端支持（HBuilderX 2.3.3+）|
| sslVerify| Boolean| No| true| Verify ssl certificate| App-Android only (HBuilderX 2.3.3+)|
...
必须大写，有效值在不同平台差异说明不同。
Valid values must be capitalized and are different in each platform difference description.
...
1. File path format: You can put the certificate file into the‘static' directory of the project, and then fill in the file path, for example: `'/static/client.p12'`
