...
- Maps opened by different platforms are different. The App opens the Gaode map (Baidu map is not supported) and the H5 side opens the Tencent map.
...
- 点击返回也会进入 `fail` 回调中
- Clicking return will also enter the `fail` callback
