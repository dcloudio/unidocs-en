...
subNvue, the native child form of vue page, covers the native interface rendered by weex as a sub-child form of vue page on the page. It is not a full-screen page. It provides a more powerful and flexible solution for hierarchical coverage and native interface custom in the vue page of App platform. It is just a native child form, not a component.

它的设计背景和用途详见：[subNVue 原生子窗体开发指南](https://ask.dcloud.net.cn/article/35948)
Its design background and usage are detailed in: [subNVue Native Sub-form Development Guide](https://ask.dcloud.net.cn/article/35948)
...
An instance of obtaining the `subNVues` native sub-form by `ID`.  [subNVues configuration](/collocation/pages?id=app-subNVues)
...
An instance of obtaining the current `subNVues` native sub-form in the nvue page code of a subnvue form.
...
#### Method form of subNVue object @subNVue

|方法			|说明				|
| Method| Instruction|
|---			|---				|
|show			| 显示原生子窗体		|
| show| Display native child form|
|hide			| 隐藏原生子窗体		|
...
| duration| Number| 600| Display the animation duration of the native child form, in ms. If it is not set, the default window animation time will be 600 ms.|
|showedCB	|Function|		|显示完成的回调函数，当指定原生子窗体显示动画执行完毕时触发回调函数，窗口无动画效果（如"none"动画效果）时也会触发此回调。	|
| showedCB| Function| | Display the finished callback function. The displayed callback function will be triggered when the specified native child form display animation is finished, and this callback will also be triggered when the window has no animation effect (such as "none" animation effect).|
...
| duration| Number| 600| Hide the animation duration of the native child form, in ms. If it is not set, the default window animation time will be 600 ms.|
...
| style| Object| Style of the native child form|

**原生子窗体的样式**
**Style of the native child form**
...
There will be default corresponding rules for turning on/off the animation. However, if the closing animation type of the native child form is generated through API, the default type will not be used.
...
- To learn more about the purpose of `subNVue` native sub-forms, please read the [Development guide for subNVues native sub-form](https://ask.dcloud.net.cn/article/35948)
- 在hello uni-app的接口-界面-原生子窗体中，有完整的subNVue示例，包括带渐变色的原生导航栏、可遮盖地图的侧滑菜单、可从顶部弹出的popup、可在视频上滚动的消息列表
- In the port-interface-native child form of hello uni-app, there are complete examples of subNVue, including native navigation bar with gradient color, sideslip menu that can cover the map, popup that can pop out from the top, and message list that can scroll on the video
...
- `subNVue` is more powerful than cover-view and takes up more memory. To ensure a better performance experience, do not load too many `subNVue` sub-forms on a vue page. It is recommended that no more than three sub-forms are loaded.
