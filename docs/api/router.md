...
// On pages/test.vue, transfer data to the startup page via events
onLoad: function(option) {
  console.log(option.query)
  // #ifdef APP-NVUE
  // 兼容APP-NVUE
  //compatible with APP-NVUE
...
* The target pages of the routing API must be the vue pages registered in pages.json. To open the web url, the web-view component on the App platform and the window.open on the H5 platform can be used. There is a component ulink.vue in hello uni-app that has been encapsulated in multiple sides for reference.
...
> This API is only supported in App. For the window animation on the H5 side, a conventional single-page animation processing scheme can be used, see [Single-page animation example on the H5 side](https://ext.dcloud.net.cn/plugin?id=659&tdsourcetag=s_pctim_aiomsg)
