...
<script>
	...
	//jql.vue component
	// 如果符合easycom规范，无需本代码
	//If it conforms to the easycom specification, this code is not required
...
export default {
	...
	data() {
		return {
			...
			//Request the returned data, which will update after calling loadData
			mixinDatacomResData: [],
			// 请求出错时的错误消息
			//error messages when the request failed
			mixinDatacomErrorMessage: '',
			// 分页信息，详情见 created 生命周期
			// For paging information, see created life cycle for details
			mixinDatacomPage: {}
		}
	},
	created() {
		this.mixinDatacomPage = {
			...
			//current page, pageCurrent in initialization setting of props
			current: this.pageCurrent,
			// 页面大小，初始化设置 props中的 pageSize
			//page size, pageSize in initialization setting of props
			size: this.pageSize,
			// 数据总数，getcount=true时有效
			//total data, which is valid when getcount=true
			count: 0,
		}
		this.$watch(() => {
			var al = [];
