...
| sizeType| Array\<String>| No| original refers to original images and compressed refers to compressed images. Both exist by default| App|
...
| fail| Function| No| Callback function for failed interface calling| App|
...
| quality| Number| No| The value ranges from 1 to 100 and the smaller the value, the lower the quality (This parameter is valid only for jpg format). 80 as default.|
|width|Number|是|裁剪的宽度，单位为px，用于计算裁剪宽高比。|
| width| Number| Yes| Cropping width, in px, used to calculate crop width-to-height ratio.|
|height|Number|是|裁剪的高度，单位为px，用于计算裁剪宽高比。|
| height| Number| Yes| Cropping height, in px, used to calculate crop width-to-height ratio.|
|resize|Boolean|否|是否将width和height作为裁剪保存图片真实的像素值。默认值为true。注：设置为false时在裁剪编辑界面显示图片的像素值，设置为true时不显示|
| resize| Boolean| No| Whether to use the width and height as the true pixel value of the saved cropped image. The default value is true. Note: If false, it displays the pixel value of the image on the cropping editing interface. If true, it doesn't display.|
...
- The sourceType corresponds to the `capture` attribute of `input` on the H5 side. Setting it to `['album']` is invalid, but the camera can still be used.
...
- App端如需选择非媒体文件，可在插件市场搜索[文件选择](https://ext.dcloud.net.cn/search?q=文件选择)。
- If you need to select non-media files on the App side, you can search for [File selection](https://ext.dcloud.net.cn/search?q=%E6%96%87%E4%BB%B6%E9%80%89%E6%8B%A9) in the plug-in market.
...
uni.chooseImage({
	...
	count: 6,
	//可以指定是原图还是压缩图，默认二者都有
	// You can specify the image original or compressed or both.
...
- Upload the index values, and the original urls will be preserved without dereplication.
...
- If uplod B2 index value 3, the preview result is B2, the previous one is C, and the next one is D.
...
uni.chooseImage({
	...
	success: function(res) {
		...
		uni.previewImage({
			...
			longPressActions: {
				itemList: ['Send to friend', 'Save image', 'Favorites'],
				success: function(data) {
					console.log('select' + (data.tapIndex + 1) + 'th button, and ' + (data.index + 1) + 'th image');
				},
				fail: function(err) {
...
| orientation| String| Orientation of the returned image. The valid values are listed in the table below| App|
|type|String|返回图片的格式|App|
| type| String| Return the format of the image| App|
...
| width| String| auto| No| Width of the image to be scaled, in pixel (e.g., "100px"), percentage (e.g., "50%") or automatic calculation (e.g., "auto", based on the scaling ratio of height to the height of the source image. If height is not set, use the height of the source image).| App 3.0.0+|
...
| rotate| Number| 0| No| Rotation degree, ranging from 0 to 360.| App 3.0.0+| |
| success | Function |  | 否 | 接口调用成功的回调函数 ||
| success| Function| | No| Callback function for successful interface calling| |
| fail | Function |  | 否 | 接口调用失败的回调函数 ||
| fail| Function| | No| Callback function for failed interface calling| |
