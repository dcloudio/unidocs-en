...
- App supports multi-file upload, and this API needs to be called repeatedly to upload multiple files. Therefore, the cross-platform writing mode is to call this API circularly.
...
uni.chooseImage({
	success: (chooseImageRes) => {
		...
		uploadTask.onProgressUpdate((res) => {
			...
			console.log('total length of expected data to be uploaded'+ res.totalBytesExpectedToSend);
...
downloadTask.onProgressUpdate((res) => {
	...
	console.log('length of downloaded data '+ res.totalbyteswriten);
	console.log('total length of expected data to be downloaded'+ res.totalBytesExpectedToSend);

	// 测试条件，取消下载任务。
	// Testing condition, to cancel the download task.
	if (res.progress > 50) {
		downloadTask.abort();
	}
});
